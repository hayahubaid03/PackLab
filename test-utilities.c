...
int test_lfsr_step(void) {
  ...
  uint16_t correct_lfsr_states[16] = {0x1337, 0x099B, 0x84CD, 0x4266,
...
int parseh_test(void) {
  uint8_t input_data[]={0x02, 0x013, 0x02, 0x00};
  bool expected = true;

  packlab_config_t* config = malloc_and_check(sizeof(packlab_config_t));
  config->is_valid = false;
  parse_header(input_data, sizeof(input_data), config);

  if (config-> is_valid != expected){
    free(config);
    return 1;
  }
  free(config);
